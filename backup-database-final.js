...
async function backupDatabase() {
  console.log('Starting comprehensive database backup (final version)...\n');
  
  try {
    // Use raw SQL queries to avoid schema validation issues
    const tables = [
      'Herb',
      'Symptom', 
      'SymptomVariant',
    ...
    for (const table of tables) {
      ...
      try {
        // Use raw SQL to get all data without schema validation
    ...
    const filename = `database_backup_final_${timestamp}.json`;
    
    fs.writeFileSync(filename, JSON.stringify(backup, null, 2));
    console.log(`\n✓ Backup saved to: ${filename}`);
    
    // Also save as SQL-like format
    const sqlFilename = `database_backup_final_${timestamp}.sql`;
