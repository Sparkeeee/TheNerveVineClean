// Pacific Botanicals Batch Scraper - World #7
...
const FILTER_KEYWORDS = [
  'pasionflower',      // Pacific Botanicals format (one word)
  'passionflower',     // Alternative format (one word)
  'passion-flower',    // Hyphenated format
  'passiflora',        // Scientific name
  // Add more keywords as needed:
  // 'ashwagandha',    // Ashwagandha products
  // 'ginkgo',         // Ginkgo products
...
const PROGRESS_FILE = "./pacific_progress.json";
...
// === SITEMAP DISCOVERY ===
async function getPacificSitemapUrls() {
  const indexUrl = "https://pacificbotanicals.com/sitemap.xml";
  const allUrls = [];
  ...
  const productUrls = [...new Set(allUrls)].filter((u) =>
    /^https:\/\/pacificbotanicals\.com\/products\//i.test(u)
  );
    ...
    const crawler = new PlaywrightCrawler({
      ...
      async requestHandler({ page, request }) {
        try {
          ...
          // Extract Pacific Botanicals product data
          const data = await page.evaluate(() => {
            const name = document.querySelector("h1.product__title")?.textContent?.trim() ?? null;
            const price = document.querySelector(".price-item--regular")?.textContent?.trim() ?? null;
            ...
            const altPrice = document.querySelector(".price, [data-price]")?.textContent?.trim() ?? null;
...
async function main() {
  try {
    log.info('🚀 Starting Pacific Botanicals Batch Scraper...');
    ...
    if (urls.length === 0) {
      ...
      urls = await getPacificSitemapUrls();
    } else {
...
module.exports = { main, getPacificSitemapUrls, runBatchCrawler };
