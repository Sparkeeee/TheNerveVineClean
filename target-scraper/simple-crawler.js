...
class ManualIHerbCrawler {
    constructor(options = {}) {
        this.outputFile = options.outputFile || 'iherb_kava_root_urls.json';
    ...
    async run() {
        ...
        try {
            ...
            console.log('✅ Browser launched successfully!');
                    ...
                    // Start with more specific Kava root search
        const startUrl = 'https://www.iherb.com/search?kw=kava%20root';
    ...
    async processPage(page, url) {
        try {
            ...
            if (isBlocked) {
                ...
                console.log('3. Wait for the normal iHerb page to load');
                        ...
                        if (href && !href.includes('#') && !href.includes('javascript:')) {
                            ...
                            if (!href.startsWith('http')) {
                                fullUrl = href.startsWith('/') ? 
                                    `https://www.iherb.com${href}` : 
                                    `https://www.iherb.com/${href}`;
                            }
                            // Only keep actual product URLs
                            if (fullUrl.includes('/pr/')) {
                                links.push(fullUrl);
                            }
                        }
                    });
...
async function main() {
    ...
    const crawler = new ManualIHerbCrawler({
        maxPages: 3,
        outputFile: 'iherb_kava_root_urls.json'
    });
