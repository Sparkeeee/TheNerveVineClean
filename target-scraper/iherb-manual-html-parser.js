...
class IHerbManualHTMLParser {
  constructor() {
    this.config = {
      selectors: {
        productLinks: "a[href*='/product/'], a[href*='/p/'], a[href*='/vitamins/'], a[href*='/supplements/'], a[href*='/herbs/']",
        productGrid: "[data-test*='product'], .product-grid, .search-results, .product-list, .product-item",
        nextPage: "a[aria-label*='Next'], button[aria-label*='Next'], a:contains('Next'), .pagination a:last-child"
      },
      baseUrl: "https://www.iherb.com",
      outputDir: "./iherb_results"
    };
  }

  parseHTML(htmlContent) {
    console.log("🔍 Parsing iHerb HTML content...");
    ...
    // Strategy 4: Look for iHerb-specific product patterns
    const iherbProductRegex = /href=["']([^"']*\/[A-Z0-9]{4,}[^"']*)["']/gi;
    while ((match = iherbProductRegex.exec(htmlContent)) !== null) {
      let url = match[1];
      if (url.startsWith('/')) {
        url = this.config.baseUrl + url;
      }
      // Filter for likely product URLs (iHerb uses alphanumeric product codes)
      if (url.includes('/') && /\/[A-Z0-9]{4,}/.test(url)) {
        urls.add(url);
      }
    }
  ...
  async saveResults(urls, searchTerm) {
    ...
    const baseFilename = `iherb_results_${searchTerm.replace(/\s+/g, '_')}_${timestamp}`;
    ...
    const txtContent = `iHerb Product URLs - ${searchTerm}\nGenerated: ${new Date().toISOString()}\nTotal URLs: ${urls.length}\n\n${urls.join('\n')}`;
  ...
  async interactiveMode() {
    ...
    console.log("🎯 iHerb Manual HTML Parser - Interactive Mode");
...
async function main() {
  const parser = new IHerbManualHTMLParser();
...
module.exports = { IHerbManualHTMLParser };
