    ...
    return (<>
      ...
      <header className="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden custom900:flex flex-1 items-center justify-between w-full">
              <nav className="flex space-x-8">
                {navigation.map((item) => (<Link key={item.name} href={item.href} className={`font-sans text-gray-700 hover:text-green-800 px-3 py-2 text-lg font-semibold transition-colors duration-200 ${pathname === item.href ? 'border-b-2 border-green-700 font-bold' : ''}`}>
                    {item.name}
                  </Link>))}
              </nav>
            ...
            <div className="custom900:hidden">
              <button type="button" className="text-gray-700 hover:text-green-800 p-2" onClick={() => setIsMenuOpen(!isMenuOpen)}>
          ...
          {/* Mobile Navigation */}
          {isMenuOpen && (<div className="custom900:hidden">
