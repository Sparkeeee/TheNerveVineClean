...
export default function SupplementPage(_a) {
    return __awaiter(this, arguments, void 0, function* ({ params }) {
        ...
        if (Array.isArray(supplement.productFormulations) && supplement.productFormulations.length > 0) {
            const validProducts = Array.isArray(supplement.productFormulations) ? supplement.productFormulations.filter((p) => !!(p && typeof p === 'object' && 'name' in p && 'description' in p && 'price' in p && 'tags' in p && 'affiliateLink' in p)) : [];
            productCards = validProducts.map((product, idx) => (<div key={idx} className="border border-green-200 rounded-lg p-4 hover:shadow-md transition-shadow mb-4 bg-white">
        ...
        <div className="flex flex-wrap gap-2 mb-2">
          {product.tags.map((tag, i) => (<span key={i} className="bg-green-100 text-green-800 px-2 py-1 rounded text-xs border border-green-300">{tag}</span>))}
        </div>
