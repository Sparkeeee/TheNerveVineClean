      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {Object.entries(supplements).map(([slug, data]) => {
            var _a, _b;
            return (<Link key={slug} href={`/supplements/${slug}`} className="bg-white/80 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-blue-100 hover:shadow-xl transition-all duration-200 hover:scale-105">
              <h3 className="text-xl font-semibold text-blue-800 mb-2">{(_a = data.title) !== null && _a !== void 0 ? _a : ''}</h3>
              <p className="text-gray-600 text-sm">{(_b = data.subtitle) !== null && _b !== void 0 ? _b : ''}</p>
            </Link>);
        })}
        </div>
