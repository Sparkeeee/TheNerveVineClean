        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {sortedHerbs.map((herb, index) => (<div key={index} className="bg-white/80 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-blue-100 hover:shadow-xl transition-all duration-200 hover:scale-105">
              ...
              <p className="text-gray-500 text-sm italic mb-2">
                {('latinName' in herb ? herb.latinName : getLatinName(herb.description))}
              </p>
              ...
              {/* Traditional Uses */}
              <div className="flex flex-wrap gap-2 mt-4">
                {herb.traditionalUses && herb.traditionalUses.map((use, i) => (<span key={i} className="inline-block px-3 py-1 rounded-full border text-xs font-semibold mr-2 mb-2 bg-gray-100 text-gray-700 border-gray-200">
                    {use}
                  </span>))}
              </div>
            </div>))}
