...
import { qualitySpecs, type QualitySpecification } from '../../../lib/product-quality-specs';
...
export default function QualitySettingsPage() {
  ...
  const [customSpecs, setCustomSpecs] = useState<QualitySpecification[]>([]);
  ...
  const currentSpecs = [...qualitySpecs, ...customSpecs].filter(spec => 
    spec.herbSlug === selectedHerb && spec.productType === selectedProductType
  );
  
  const handleEditSpec = (spec: QualitySpecification) => {
    setEditingSpec({ ...spec });
  };
  ...
  const handleSaveSpec = () => {
    ...
    const existingSpecIndex = customSpecs.findIndex(spec => 
      spec.herbSlug === editingSpec.herbSlug && spec.productType === editingSpec.productType
    );
  ...
  const handleDeleteSpec = (specToDelete: QualitySpecification) => {
    setCustomSpecs(customSpecs.filter(spec => 
      !(spec.herbSlug === specToDelete.herbSlug && spec.productType === specToDelete.productType)
    ));
  };
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100">
      <div className="max-w-6xl mx-auto p-6">
        ...
        {selectedHerb && selectedProductType && (
          <div className="mb-8">
            ...
            {currentSpecs.length > 0 ? (
              <div className="space-y-4">
                {currentSpecs.map((spec, index) => {
                  const isCustomSpec = customSpecs.some(customSpec => 
                    customSpec.herbSlug === spec.herbSlug && customSpec.productType === spec.productType
                  );
                  
                  return (
                    <div key={`${spec.herbSlug}-${spec.productType}-${index}`} className="border border-gray-700 rounded-lg p-4 bg-gray-800">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1 mt-1">
                            {spec.requiredTerms.map(term => (
                              <span key={term} className="bg-red-900 text-red-200 px-2 py-1 rounded text-xs border border-red-700">
                                {term}
                              </span>
                            ))}
                          </div>
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1 mt-1">
                            {spec.preferredTerms.map(term => (
                              <span key={term} className="bg-green-900 text-green-200 px-2 py-1 rounded text-xs border border-green-700">
                                {term}
                              </span>
                            ))}
                          </div>
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1 mt-1">
                            {spec.avoidTerms.map(term => (
                              <span key={term} className="bg-yellow-900 text-yellow-200 px-2 py-1 rounded text-xs border border-yellow-700">
                                {term}
                              </span>
                            ))}
                          </div>
            ...
            <div className="bg-gray-800 border border-gray-700 p-6 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    type="text"
                    value={editingSpec.requiredTerms.join(', ')}
                    onChange={(e) => setEditingSpec({
                      ...editingSpec,
                      requiredTerms: e.target.value.split(',').map(term => term.trim()).filter(Boolean)
                    })}
                    className="w-full p-2 border border-gray-600 rounded bg-gray-700 text-gray-100 focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={editingSpec.preferredTerms.join(', ')}
                    onChange={(e) => setEditingSpec({
                      ...editingSpec,
                      preferredTerms: e.target.value.split(',').map(term => term.trim()).filter(Boolean)
                    })}
                    className="w-full p-2 border border-gray-600 rounded bg-gray-700 text-gray-100 focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={editingSpec.avoidTerms.join(', ')}
                    onChange={(e) => setEditingSpec({
                      ...editingSpec,
                      avoidTerms: e.target.value.split(',').map(term => term.trim()).filter(Boolean)
                    })}
                    className="w-full p-2 border border-gray-600 rounded bg-gray-700 text-gray-100 focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="number"
                      value={editingSpec.priceRange.min}
                      onChange={(e) => setEditingSpec({
                        ...editingSpec,
                        priceRange: { ...editingSpec.priceRange, min: parseFloat(e.target.value) }
                      })}
                      className="w-full p-2 border border-gray-600 rounded bg-gray-700 text-gray-100 focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      value={editingSpec.priceRange.max}
                      onChange={(e) => setEditingSpec({
                        ...editingSpec,
                        priceRange: { ...editingSpec.priceRange, max: parseFloat(e.target.value) }
                      })}
                      className="w-full p-2 border border-gray-600 rounded bg-gray-700 text-gray-100 focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                    />
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="number"
                      step="0.1"
                      value={editingSpec.ratingThreshold}
                      onChange={(e) => setEditingSpec({
                        ...editingSpec,
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      value={editingSpec.reviewCountThreshold}
                      onChange={(e) => setEditingSpec({
                        ...editingSpec,
                        reviewCountThreshold: parseInt(e.target.value)
                      })}
                      className="w-full p-2 border border-gray-600 rounded bg-gray-700 text-gray-100 focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                    />
                  </div>
