...
export default function AdminContentDashboard() {
        ...
        if (tab === "Herbs") {
          ...
          <td className="p-2">
            {item.indications && item.indications.length > 0 ? (<div className="flex flex-wrap gap-1">
                {item.indications.map((sym) => (<span key={sym.id} className="bg-blue-900 text-blue-200 px-2 py-1 rounded text-xs border border-blue-700">
                    {sym.name}
                  </span>))}
    ...
    function renderFormFields() {
        ...
        <div className="mb-4">
          ...
          <select multiple className="w-full p-2 rounded bg-gray-700 text-gray-100 border border-gray-600 h-32" value={formData.indications || []} onChange={e => {
                    ...
                    setFormData(Object.assign(Object.assign({}, formData), { indications: selected }));
                }}>
    ...
    return (<div className="min-h-screen text-gray-100 p-8 relative" style={{
      ...
      <div className="flex space-x-4 mb-6">
        {TABS.map((t) => (<button key={t} className={`px-4 py-2 rounded ${tab === t ? "bg-blue-600 text-white" : "bg-gray-700 text-gray-300"}`} onClick={() => setTab(t)}>
            {t}
          </button>))}
      </div>
      {loading ? (<div>Loading {tab}...</div>) : error ? (<div className="text-red-400">{error}</div>) : (<div>
          ...
          {showForm && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <form className="bg-gray-800 p-8 rounded shadow-lg min-w-[320px] max-w-[90vw] max-h-[90vh] overflow-y-auto flex flex-col" onSubmit={handleFormSubmit} style={{ boxSizing: 'border-box' }}>
                ...
                <div className="flex space-x-4 mt-4">
                  ...
                  <button type="button" className="px-4 py-2 bg-gray-600 rounded hover:bg-gray-700" onClick={() => setShowForm(false)}>
                    Cancel
                  </button>
                </div>
              </form>
